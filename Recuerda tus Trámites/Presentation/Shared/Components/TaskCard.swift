...
struct CheckCircle: View {
    ...
    var body: some View {
        ZStack {
            Circle()
                .fill(isCompleted ? Color.tramiteGreen : Color(hex: "#2C2C2E"))
                .frame(width: 28, height: 28)

            Circle()
                .strokeBorder(
                    isCompleted ? Color.tramiteGreen : Color(hex: "#545456"),
                    lineWidth: 1.5
                )
                .frame(width: 28, height: 28)
...
#Preview {
    let container = TramiteModelContainer.preview
    let ctx = container.mainContext

    let t1 = TramiteTask(title: "Enviar factura de mayo", scheduledDate: Date(), priority: .must)
    t1.scheduledTime = Calendar.current.date(bySettingHour: 8, minute: 0, second: 0, of: Date())
    let t2 = TramiteTask(title: "Revisar propuesta Q3 con el equipo", scheduledDate: Date(), priority: .high)
    ...
    let t3 = TramiteTask(title: "Responder emails pendientes", scheduledDate: Date(), priority: .medium)
    let t4 = TramiteTask(title: "Leer 20 páginas", scheduledDate: Date(), priority: .low)
