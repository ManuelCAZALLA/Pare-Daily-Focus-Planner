...
struct AddTaskSheet: View {
                            ...
                            Button {
                                ...
                                VStack(spacing: 4) {
                                    ...
                                    Group {
                                        if isToday {
                                            Text("Hoy")
                                                .foregroundStyle(isSelected ? .white.opacity(0.85) : Color.tramiteGreen)
                                        } else if isTomorrow {
                                            Text("Mañana")
                                                .foregroundStyle(isSelected ? .white.opacity(0.85) : Color.tramiteGreen)
                                        } else {
                                ...
                                .background(
                                    RoundedRectangle(cornerRadius: 14)
                                        .fill(
                                            ...
                                            ? Color.tramiteGreen
                                            : isToday
                                                ? Color.tramiteGreen.opacity(0.08)
                                                : Color(hex: "#1A1A1C")
                                        )
                                        .overlay(
                                            RoundedRectangle(cornerRadius: 14)
                                                .strokeBorder(
                                                    ...
                                                    isToday ? Color.tramiteGreen.opacity(0.3) :
                                                    Color(hex: "#2A2A2C"),
                                                    lineWidth: 0.8
                                                )
                                        )
                                        .shadow(
                                            color: isSelected ? Color.tramiteGreen.opacity(0.35) : .clear,
                                            radius: 8, x: 0, y: 4
                                        )
                                )
                            }
            ...
            VStack(spacing: 0) {
                Button {
                    ...
                    HStack(spacing: 12) {
                        ZStack {
                            Circle()
                                .fill(hasTime ? Color.tramiteGreen.opacity(0.15) : Color(hex: "#2A2A2C"))
                                .frame(width: 34, height: 34)
                            Image(systemName: hasTime ? "clock.fill" : "clock")
                                .font(.system(size: 14, weight: .semibold))
                                .foregroundStyle(hasTime ? Color.tramiteGreen : Color(hex: "#8E8E93"))
                        }
        ...
        VStack(alignment: .leading, spacing: 10) {
            ...
            Menu {
                ...
                HStack(spacing: 12) {
                    ZStack {
                        Circle()
                            .fill(reminder == nil ? Color(hex: "#2A2A2C") : Color.tramiteGreen.opacity(0.15))
                            .frame(width: 34, height: 34)
                        Image(systemName: reminder == nil ? "bell.slash" : "bell.fill")
                            .font(.system(size: 14, weight: .semibold))
                            .foregroundStyle(reminder == nil ? Color(hex: "#8E8E93") : Color.tramiteGreen)
                    }
    ...
    private func save() {
        ...
        if let task = editingTask {
            ...
            let task = TramiteTask(title: trimmed, scheduledDate: scheduledDate, priority: priority)
            task.notes         = notes.isEmpty ? nil : notes
            task.scheduledTime = hasTime ? mergedDateTime() : nil
            task.alertOffset   = hasTime ? reminder : nil
            task.recurrenceRaw = hasRecurrence ? encodeRecurrence(recurrence) : nil
    ...
    private func populate(from task: TramiteTask) {
        title         = task.title
        notes         = task.notes ?? ""
...
#Preview("Nueva Tarea") {
    ...
    let s1 = TramiteTask(title: "Ir al gym", scheduledDate: Date(), priority: .medium)
    s1.scheduledTime = Calendar.current.date(bySettingHour: 18, minute: 0, second: 0, of: Date())
    let s2 = TramiteTask(title: "Ver serie", scheduledDate: Date(), priority: .low)
...
#Preview("Editar Tarea") {
    let context = TramiteModelContainer.preview.mainContext
    let task = TramiteTask(title: "Revisar propuesta Q3", scheduledDate: Date(), priority: .high)
