...
final class DayViewModel {
    ...
    var tasksToday: [TramiteTask] = []
    var selectedDate: Date = Date()
    var streak: Int = 0
    var overdueFromYesterday: [TramiteTask] = []
    var suggestions: [TramiteTask] = []
    ...
    func complete(_ task: TramiteTask) {
    ...
    func reschedule(_ task: TramiteTask, to date: Date) {
    ...
    func addTask(_ task: TramiteTask) {
        try? taskRepository.save(task)
        notificationService.schedule(for: task)
        loadDay(for: selectedDate)
    }

    func deleteTask(_ task: TramiteTask) {
        notificationService.cancel(for: task)
    ...
    private func sortTasks(_ tasks: [TramiteTask]) -> [TramiteTask] {
